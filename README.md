...
node tools/preview/smoke.js                        # منطق الواجهة (66 فحصًا)
...
- **قارئ قرآن كريم أوفلاين**: النص العثماني الكامل (مشروع Tanzil، CC-BY 3.0)، فهرس
  ...
  **مواضع السجود الخمسة عشر** مرسومة بعلامة محراب بجانب رقم الآية (`SajdahMarkerSpan`)، وتظهر
  شارة ذهبية أسفل الصفحة في الصفحات الـ15 التي تحمل سجدة، والنقر عليها يشرح السجدة
  (واجبة/مستحبة) وينقلك إلى الآية.
  **ست خلفيات قراءة** (`QuranTheme.STYLES`: ورق · رَقّ · زيتوني · أخضر · ليل · أزرق ليلي)،
  كل واحدة بتدرّجها وإطارها من `res/drawable/quran_bg_*.xml`، تُحفظ في `QuranStore`
  وتُعاد في المكان بلا فقدان لموضع القراءة.
