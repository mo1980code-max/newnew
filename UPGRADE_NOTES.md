...
## 25) إصلاح خلفية الخلفية الحية: الصورة الجديدة لا تظهر (19 سبتمبر 2026)

**العَرَض:** بعد اختيار صورة جديدة من المعرض (أو خلفية من الكتالوج، أو التبديل بين صورة/خلفية
مدمجة/لون صلب) تبقى الخلفية الحية `LiveClockWallpaper` تعرض الخلفية القديمة حتى يُعاد إنشاء
محرك الخلفية.

**السبب (ثلاثة عيوب في `LiveClockWallpaper.ClockEngine`):**

1. الصورة المفكوكة `Bitmap aa` كانت تُخزَّن ولا يُعاد تحميلها أبدًا: `safeDecodeWithCache()` تعيد
   `aa` متى ما كانت غير فارغة دون أن تتحقق **من أي ملف** فُكّت — فتغيّر `ImageString` لا يُلاحَظ.
   والأسوأ أن `SetWallpaperActivity` تكتب دائمًا نفس المسار `files/wallpapers/wallpaper.jpg`،
   فحتى مقارنة المسار وحدها ما كانت لتكفي.
2. عند التبديل بين `isImage` و`isCustomBg` واللون الصلب لا يُمسح كل ما سبق من حالة `imageView`:
   لون الخلفية الذي يضعه فرع اللون الصلب (`setBackgroundColor`) لم يكن يُزال في فرعَي الصورة
   والخلفية المدمجة.
3. `onVisibilityChanged(true)` — وهو ما يحدث تمامًا لحظة عودة المستخدم من المحرر إلى الشاشة
   الرئيسية — لم يكن يُبطل أي حالة مخزّنة.

**الإصلاح (`LiveClockWallpaper.java` فقط، دون مساس بقياس الساعة أو موضعها أو تخطيط الودجات):**

- `applyBackground()` (تُستدعى من `firstClock`) صارت ثلاثة فروع صريحة تطابق ما يعاينه
  `EditorActivity.updateClock()` بنفس سلسلة التوافق للحالات القديمة:
  - `isImage`: يُرمى `aa` القديم (فصله عن الـ ImageView أولًا ثم `recycle`) ويُفكّ الملف من جديد
    بـ `BitmapFactory.decodeFile` ثم يوضع في `imageView` ويُمسح أي لون خلفية سابق.
  - `isCustomBg`: `aa = null` (مع تدويره)، `setImageBitmap(null)`، مسح لون الخلفية، ثم
    `setImageResource(customBg)`. وإن لم يعد المعرّف يشير إلى drawable (معرّفات الموارد قد تتغير
    بين إصدارات البناء) يُهبَط إلى اللون الصلب بدل رسم لا شيء.
  - وإلا (لون صلب): `aa = null`، `setImageDrawable(null)`، ثم `setBackgroundColor(bgColor)`.
- مفتاح الذاكرة المؤقتة صار **(المسار، حجم الملف، وقت التعديل)**: فأي اختيار جديد من المعرض،
  أو إعادة كتابة `wallpaper.jpg` في مكانه، يُكتشف في الإطار التالي حتى لو لم تتغير الرؤية.
  وبين الإطارات يُعاد استخدام الصورة نفسها (الحلقة تصل إلى 30 إطارًا/ثانية في الوضع السلس،
  ففكّ الصورة في كل إطار غير وارد).
- `onVisibilityChanged(true)` يستدعي `invalidateBackgroundCache()` فيُرمى `aa` وتُقرأ إعدادات
  TinyDB من جديد ويُعاد الرسم فورًا.
- فكّ الصورة صار بعيّنة `inSampleSize` (قوى 2) تُبقي كلا البُعدين ≥ أكبر بُعد للشاشة، فلا يُكبَّر
  `CENTER_CROP` شيئًا في أي اتجاه، بينما صورة 48 ميجابكسل من الكاميرا لم تعد تحتاج ~200 م.ب
  (كانت تفشل بـ OOM فتظهر خلفية اللون الصلب بدلها).
- ملف تعذّر فكّه (تالف أو OOM حتى بعد التعيينة) لا يُعاد تجريبه في كل إطار: يُذكر مفتاحه حتى
  يتغير الملف أو تعود الرؤية. وأُزيل سيل `Log.e("isImage","yes")` الذي كان يُكتب مع كل إطار.

**الفحص:** `tools/verify_java_symbols.py` و`tools/verify_resources.py` = NO ERRORS، وأُعيد
بناء الحزمة `Allah-Clock-Live-Wallpaper-android-studio.zip`. لا JDK في هذه البيئة، فأول ما يُجرَّب
على الجهاز: اختيار صورة من المعرض ثم الرجوع للشاشة الرئيسية، ثم خلفية مدمجة، ثم لون صلب،
ثم صورتان متتاليتان من الكتالوج (نفس المسار) — يجب أن تظهر كل خلفية فور الرجوع.
