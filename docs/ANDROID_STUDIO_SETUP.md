...
## 7) أدوات التحقق داخل المستودع (بلا JDK)
...
## 8) نتيجة الفحص الكامل للمشروع (16 سبتمبر 2026)

| البند | النتيجة |
|---|---|
| ملفات XML في `res/` (85) | كلها تُحلَّل بلا خطأ |
| النصوص: `values` مقابل `values-ar` | **106 / 106** متطابقة، ووسائط التنسيق (`%1$d`) متماثلة، وشرطات الاقتباس مُهرَّبة |
| المصفوفات | 7 في الافتراضي و6 في العربية، وكل المشتركة بنفس عدد العناصر (`adhkar` عربية عن قصد في اللغتين) |
| مراجع `R.*` في 61 ملف جافا | كلها مُعلنة (نصوص، معرّفات، مصفوفات، رسومات، ألوان، `raw`) |
| مراجع `@type/name` داخل التخطيطات والرسومات | كلها تُحلّ إلى مورد موجود |
| `findViewById(R.id.x)` | كل هدف له `@+id/x` في تخطيط ما |
| توازن أقواس جافا (ماسح أحرف لا تعبير نمطي) | 61/61 سليمة |
| مكونات Manifest (15) | كل صنف موجود، و`android:exported` معرّف على كل مكوّن (مطلوب من targetSdk 31+) |
| حزم جافا مقابل مسارات المجلدات | 61/61 مطابقة |
